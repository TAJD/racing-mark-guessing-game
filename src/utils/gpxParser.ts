...
export function parseGpxData(gpxContent: string): RacingMark[] {
  ...
  for (let i = 0; i < waypoints.length; i++) {
    ...
    if (nameElement && symElement && descElement) {
      ...
      const sponsor = description.includes('*') || description.includes('@') ? 
        description.split(/[*@]/)[0].trim() : undefined;
...
export function getMarksByDifficulty(marks: RacingMark[], difficulty: 'beginner' | 'intermediate' | 'advanced'): RacingMark[] {
  switch (difficulty) {
    case 'beginner':
      // Famous landmarks and well-known marks (those without sponsors)
      return marks.filter(mark => !mark.sponsor && 
...
export function findNearbyMarks(targetMark: RacingMark, allMarks: RacingMark[], radiusMeters: number): RacingMark[] {
